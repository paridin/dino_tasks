defmodule Mix.Tasks.Dino.Gen.Context do
  ...
  defp inject_schema_file(%Context{dir: dir, schema: schema}, paths, binding) do
    file = Path.join([dir, "#{schema.singular}.ex"])
    Mix.Generator.create_file(file, Mix.Phoenix.eval_from(paths, "priv/templates/dino.gen.context/schema.ex", binding))
  end

  defp inject_schema_access(%Context{file: file} = context, paths, binding) do
    unless Context.pre_existing?(context) do
      Mix.Generator.create_file(file, Mix.Phoenix.eval_from(paths, "priv/templates/dino.gen.context/context.ex", binding))
    end

    paths
    |> Mix.Phoenix.eval_from("priv/templates/dino.gen.context/#{schema_access_template(context)}", binding)
  ...
  defp inject_tests(%Context{test_file: test_file} = context, paths, binding) do
    unless Context.pre_existing_tests?(context) do
      Mix.Generator.create_file(test_file, Mix.Phoenix.eval_from(paths, "priv/templates/dino.gen.context/context_test.exs", binding))
    end
  ...
  def raise_with_help(msg) do
    ...
    For example:
        ...
        mix dino.gen.live Accounts User users name:string
        mix dino.gen.context Accounts User users name:string
